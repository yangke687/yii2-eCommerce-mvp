...
<?php if (!empty($items)): ?>
    <table class="table table-hover">
        ...
        <?php foreach ($items as $item): ?>
            <tr data-product_id="<?= $item['product_id'] ?>" data-url="<?= \yii\helpers\Url::to('/cart/change-quantity') ?>">
                ...
                <td><?= $item['price'] ?></td>
                <td>
                    <input type="number" class="form-control chart-item-quantity" style="width: 60px;" min="1"
                           value="<?= $item['quantity'] ?>"/>
                </td>
